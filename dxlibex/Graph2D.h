	...
	namespace graph2d
	{
		...
		*/
		...
		public:
			...
			//! 画像の拡大描画
			template<typename T> inline int DrawExtendGraph(const point_c<T>& lu, const point_c<T>& rb, bool TransFlag)const DXLE_NOEXCEPT_OR_NOTHROW_SINGLE { DXLE_GET_LOCK(screen_mutex_c::mtx); if (std::is_integral<T>::value) { return DxLib::DrawExtendGraph(lu.x, lu.y, rb.x, rb.y, GetHandle(), TransFlag); } else { return DxLib::DrawExtendGraphF(lu.x, lu.y, rb.x, rb.y, GetHandle(), TransFlag); } }
			//! 画像の回転描画
			...
			//! 画像の回転描画
			template<typename T> inline int DrawRotaGraph(const point_c<T>& p, double ExRate, double Angle, bool TransFlag, bool TurnFlag = false)const DXLE_NOEXCEPT_OR_NOTHROW_SINGLE { DXLE_GET_LOCK(screen_mutex_c::mtx); if (std::is_integral<T>::value) { return DxLib::DrawRotaGraph(p.x, p.y, ExRate, Angle, GetHandle(), TransFlag, TurnFlag); } else { return DxLib::DrawRotaGraphF(p.x, p.y, ExRate, Angle, GetHandle(), TransFlag, TurnFlag); } }
			//! 画像の回転描画２( 回転中心指定付き )
			...
			//! 画像の回転描画２( 回転中心指定付き )
			template<typename T> inline int DrawRotaGraph2(const point_c<T>& p, const point_c<T>& c, double ExtRate, double Angle, bool TransFlag, bool TurnFlag = false)const DXLE_NOEXCEPT_OR_NOTHROW_SINGLE { DXLE_GET_LOCK(screen_mutex_c::mtx); if (std::is_integral<T>::value) { return DxLib::DrawRotaGraph2(p.x, p.y, c.x, c.y, ExtRate, Angle, GetHandle(), TransFlag, TurnFlag); } else { return DxLib::DrawRotaGraph2F(p.x, p.y, c.x, c.y, ExtRate, Angle, GetHandle(), TransFlag, TurnFlag); } }
			...
			//! 画像の回転描画３( 回転中心指定付き＋縦横拡大率別指定版 )
			template<typename T> inline int DrawRotaGraph3(const point_c<T>& p, const point_c<T>& c, double ExtRateX, double ExtRateY, double Angle, bool TransFlag, bool TurnFlag = FALSE)const DXLE_NOEXCEPT_OR_NOTHROW_SINGLE { DXLE_GET_LOCK(screen_mutex_c::mtx); if (std::is_integral<T>::value) { return DxLib::DrawRotaGraph3(p.x, p.y, c.x, c.y, ExtRateX, ExtRateY, Angle, GetHandle(), TransFlag, TurnFlag); } else { return DxLib::DrawRotaGraph3F(p.x, p.y, c.x, c.y, ExtRateX, ExtRateY, Angle, GetHandle(), TransFlag, TurnFlag); } }
			//! 画像の自由変形描画
			...
			//! 画像の自由変形描画
			template<typename T> inline int DrawModiGraph(const std::array<point_c<T>, 4>& ps, bool TransFlag)const DXLE_NOEXCEPT_OR_NOTHROW_SINGLE { DXLE_GET_LOCK(screen_mutex_c::mtx); if (std::is_integral<T>::value) { return DxLib::DrawModiGraph(ps[0].x, ps[0].y, ps[1].x, ps[1].y, ps[2].x, ps[2].y, ps[3].x, ps[3].y, GetHandle(), TransFlag); } else { return DxLib::DrawModiGraphF(ps[0].x, ps[0].y, ps[1].x, ps[1].y, ps[2].x, ps[2].y, ps[3].x, ps[3].y, GetHandle(), TransFlag); } }
			//! 画像の左右反転描画
			...
			//! 画像の左右反転描画
			template<typename T> inline int DrawTurnGraph(const point_c<T>& p, bool TransFlag)const DXLE_NOEXCEPT_OR_NOTHROW_SINGLE { DXLE_GET_LOCK(screen_mutex_c::mtx); if (std::is_integral<T>::value) { return DxLib::DrawTurnGraph(p.x, p.y, GetHandle(), TransFlag); } else { return DxLib::DrawTurnGraphF(p.x, p.y, GetHandle(), TransFlag); } }
			//! 画像の拡大左右反転描画
			...
			//! 画像の拡大左右反転描画
			template<typename T> inline int DrawExtendTurnGraph(const point_c<T>& lu, const point_c<T>& rb, bool TransFlag)const DXLE_NOEXCEPT_OR_NOTHROW_SINGLE { DXLE_GET_LOCK(screen_mutex_c::mtx); if (std::is_integral<T>::value) { return DxLib::DrawExtendGraph(rb.x, lu.y, lu.x, rb.y, GetHandle(), TransFlag); } else { return DxLib::DrawExtendGraphF(rb.x, lu.y, lu.x, rb.y, GetHandle(), TransFlag); } }//lu.xとrb.xはこれで正しい
