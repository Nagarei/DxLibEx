#ifndef DX_LIB_EX_IMPL_H_2015_0930_0605_1596851632_
#define DX_LIB_EX_IMPL_H_2015_0930_0605_1596851632_
...
namespace DxLibEx
{
	...
	namespace nDxLibSys
	{
		...
		DxLibSys::~DxLibSys()DXLIBEX_NOEXCEPT {
			DxLibSys::End();
		}
	}
	...
	namespace nCounter
	{
		...
		inline Counter Counter::GetNowCount(bool UseRDTSCFlag)DXLIBEX_NOEXCEPT {
			return Counter(DxLib::GetNowCount(UseRDTSCFlag));
		}
		//! 1000をかけてHiPerformanceCounterとみなすようにする
		inline HiPerformanceCounter Counter::CastToHiPerformance()const DXLIBEX_NOEXCEPT {
			return Cast<LONGLONG>::Do(static_cast<LONGLONG>(count()) * 1000);
		}
		//!マイクロ秒単位の精度を持つカウンタの現在値を得る
		inline HiPerformanceCounter HiPerformanceCounter::GetNowCount(bool UseRDTSCFlag)DXLIBEX_NOEXCEPT {
			return HiPerformanceCounter(DxLib::GetNowHiPerformanceCount(UseRDTSCFlag));
		}
		//! 1000で割ってCounterとみなすようにする
		inline Counter HiPerformanceCounter::CastToLowPerformance()const DXLIBEX_NOEXCEPT {
	...
	namespace nMouse
	{
		inline MouseState::MouseState() DXLIBEX_NOEXCEPT
		...
		inline MouseState::MouseState(int x, int y, int scrollWheel
			...
			, ButtonState xButton1, ButtonState xButton2) DXLIBEX_NOEXCEPT
			...
			, Y(y)
		{}
		inline bool MouseState::operator==(const MouseState& right)DXLIBEX_NOEXCEPT
		{
			return (
		...
		inline MouseInputInfo::MouseInputInfo()DXLIBEX_NOEXCEPT
			: X(0)
			, Y(0)
			, Type(0)
		{}
		inline MouseInputInfo::MouseInputInfo(int x, int y, int type)DXLIBEX_NOEXCEPT
			: X(x)
			, Y(y)
			, Type(type)
		{}
		...
		MouseState Mouse::GetState(bool ScrollWheelValueReset)DXLIBEX_NOEXCEPT
		{
			int x, y;
