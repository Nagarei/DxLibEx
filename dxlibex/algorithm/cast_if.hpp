...
namespace dxle {
	namespace detail {
		...
		struct static_cast_if_helper{
			DXLE_CONSTEXPR to operator()(const from& n) const DXLE_NOEXCEPT_IF_EXPR(static_cast<double>(std::declval<from>())) {
				return static_cast<double>(n);
			}
		};
		template<typename from, typename to>
		struct static_cast_if_helper<to, from, false, false>{
			DXLE_CONSTEXPR to operator()(const from& n) const DXLE_NOEXCEPT_IF(std::is_nothrow_copy_constructible<from>::value){
				return n;
			}
		};
		template<typename from, typename to>
